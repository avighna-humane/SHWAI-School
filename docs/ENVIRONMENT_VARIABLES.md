...
| Sessions/security | `SESSION_SECRET`, `JWT_SECRET`, `MFA_ENCRYPTION_KEY`, `SHWAI_TRUSTED_ORIGINS`       | Required for production session/origin policy; `MFA_ENCRYPTION_KEY` is required before production TOTP enrollment. Rotate through incident procedure. |
| Scheduled work    | `SHWAI_INTELLIGENCE_CRON_SECRET`, `SHWAI_JOB_RUNNER_SECRET`, `SHWAI_WORKER_POLL_MS` | Required before enabling scheduled endpoints/workers; use platform-native auth where available.                                                       |
...
| Storage | `STORAGE_PROVIDER`, `STORAGE_ENDPOINT`, `STORAGE_BUCKET`, `STORAGE_ACCESS_KEY_ID`, `STORAGE_SECRET_ACCESS_KEY`, `STORAGE_REGION` | Required for private object storage, signed URLs, expiry, and export artifacts; local filesystem storage is not accepted in production. |
| Malware scanning | `MALWARE_SCANNER_URL`, `MALWARE_SCANNER_API_KEY` | Required before user document downloads are enabled; non-clean objects fail closed. |
...
| Billing | `PAYMENT_PROVIDER`, `PAYMENT_API_KEY`, `PAYMENT_WEBHOOK_SECRET` | Required for the signed idempotent webhook boundary; provider-hosted checkout, customer creation, and sandbox evidence remain separate launch gates. No card data belongs in SHWAI. |
| Backups | `BACKUP_PROVIDER`, `BACKUP_BUCKET`, `BACKUP_RETENTION_DAYS`, `BACKUP_OUTPUT_DIR`, backup credentials | Required to configure backup execution and retention; readiness remains warning until provider backup and restore evidence exists. |
