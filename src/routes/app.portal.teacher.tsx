...
function TeacherPortalPage() {
  ...
  const [activeTab, setActiveTab] = useState<
    "dashboard" | "homework" | "students" | "staff-notices" | "chat"
  >("dashboard");
      ...
      <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as any)} className="space-y-4">
        <TabsList className="flex flex-wrap rounded-xl bg-card p-1 shadow-sm border border-border/50">
          ...
          <TabsTrigger value="homework" className="text-xs gap-1.5">
            <BookOpen className="size-3.5" /> My Homework ({hws.length})
          </TabsTrigger>
          <TabsTrigger value="students" className="text-xs gap-1.5">
            <Users className="size-3.5" /> Students ({students.length})
          </TabsTrigger>
          <TabsTrigger value="staff-notices" className="text-xs gap-1.5">
            <Megaphone className="size-3.5" /> Principal Posts ({staffNotices.length})
          </TabsTrigger>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <Card>
              <CardHeader className="pb-3">
                ...
                <CardDescription>
                  Track active academic metrics and workload targets.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="flex items-center justify-between rounded-xl bg-muted/30 p-3.5 border border-border/40 text-sm">
                  ...
                  <p className="text-xl font-extrabold text-danger">
                    {teacher.gradingBacklog} assignments
                  </p>
                </div>
          ...
          <Card>
            <CardHeader className="flex flex-row justify-between items-center pb-3">
              <div>
                ...
                <CardDescription>
                  Track student activity (views, submissions) and manage worksheets.
                </CardDescription>
              </div>
              <HomeworkFormDialog
                role="teacher"
            ...
            <CardContent>
              {homeworkQuery.isLoading ? (
                ...
                <EmptyState
                  ...
                  description="Assign your first homework to let students submit their work."
                    ...
                    >
                      ...
                      <div className="relative z-10">
                        ...
                        <p className="text-xs text-muted-foreground mt-0.5">
                          {hw.assignedCount ?? 0} students assigned
                        </p>
                      </div>
        ...
        <TabsContent value="students" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Track homework completion, attendance, and view student portfolios end-to-end.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {students.map((student) => (
                  <div
                    key={student.id}
                    className="border p-4 rounded-xl flex flex-col justify-between gap-3 bg-card hover:border-primary/50 transition-colors"
                  >
                    <div>
                      <div className="flex items-center gap-2.5">
                    ...
                    >
                      <SheetTrigger asChild>
                        <Button
                          size="sm"
                          variant="outline"
                          className="w-full mt-3 h-8 text-[11px] gap-1"
                      ...
                      <SheetContent className="max-w-lg overflow-y-auto sm:max-w-xl">
                        <SheetHeader className="pb-4 border-b">
                          ...
                          <SheetDescription>
                            Academic summary, homework submissions, and socratic chat portfolio.
                          </SheetDescription>
                        </SheetHeader>
                        {selectedStudent?.id === student.id && (
                          <TeacherStudentProfileDetail
                            student={student}
        ...
        <TabsContent value="staff-notices" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Directives, posts and official materials targeted exclusively to school staff.
              </CardDescription>
            </CardHeader>
            <CardContent>
              {staffNotices.length === 0 ? (
                <EmptyState
                  title="No principal posts"
                  description="There are no active staff announcements published currently."
                ...
                <div className="space-y-4">
                  {staffNotices.map((n: any) => (
                    <div
                      key={n.id}
                      className="p-4 border rounded-xl flex flex-col gap-2.5 bg-card hover:bg-muted/10 transition-colors"
                    >
                      ...
                      {n.attachments && n.attachments.length > 0 && (
                        <div className="border-t pt-3 mt-1 space-y-1.5">
                          ...
                          <AttachmentList
                            files={n.attachments}
                            getUrl={async () => n.attachments[0].filePath}
                          />
                        </div>
                      )}
                    </div>
  ...
  return (
    <div className="space-y-6 pt-4">
      ...
      <div className="grid grid-cols-2 gap-4 text-xs border rounded-xl p-3 bg-muted/20">
        <div>
          <p className="text-muted-foreground uppercase font-bold tracking-wider text-[10px]">
            ...
            <div className="space-y-4">
              {subs.map((sub) => {
                ...
                return (
                  <div
                    key={sub.id}
                    className="border p-4 rounded-xl space-y-3 bg-card flex flex-col gap-1"
                  >
                    <div className="flex justify-between items-start gap-2">
                      <div>
                        <p className="font-bold text-sm text-primary">
                          Homework ID: {sub.homeworkId.slice(0, 8)}
                        </p>
                    ...
                    {sub.files && sub.files.length > 0 && (
                      <div className="space-y-1.5 pt-1">
                        ...
                        <AttachmentList
                          files={sub.files}
                          getUrl={async () => sub.files[0].filePath}
                        />
                      </div>
                    )}
                    ...
                    {isGraded ? (
                      <div className="p-3 bg-success-soft/20 border border-success/20 rounded-lg text-xs space-y-1 mt-2">
                        <p className="font-bold text-success flex items-center gap-1">
                          <Award className="size-3.5" /> Marks: {sub.marks ?? "—"}
                        </p>
                        {sub.feedback && (
                          <p className="text-muted-foreground mt-0.5">{sub.feedback}</p>
                        )}
                      </div>
                    ) : (
                      <>
                          ...
                          >
                            <div className="grid gap-3 sm:grid-cols-3">
                              <div className="space-y-1">
                                <Label htmlFor="grade-marks-input" className="text-xs font-bold">
                                  Marks
                                </Label>
                                <Input
                              ...
                              <div className="sm:col-span-2 space-y-1">
                                <Label htmlFor="grade-feedback-input" className="text-xs font-bold">
                                  Feedback
                                </Label>
                                <Input
          ...
          >
            <div className="flex gap-2">
              ...
              <Button
                size="sm"
                type="submit"
                disabled={sendMessageMutation.isPending || (!chatInput.trim() && !attachment)}
                className="h-9"
              >
        ...
        <div className="md:col-span-2 border rounded-xl p-3 flex flex-col justify-between h-[360px] bg-card">
          {selectedConvoId ? (
            <>
              ...
              >
                <div className="flex gap-2">
                  ...
                  />
                  <Label
                    htmlFor="teacher-tab-chat-attachment"
                    className="cursor-pointer border rounded-md px-2 flex items-center hover:bg-muted shrink-0 h-9"
                  >
                    <Paperclip className="size-4 text-muted-foreground" />
                    <input
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center p-4">
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                Select or open a chat with a student to begin.
              </p>
            </div>
          )}
        </div>
