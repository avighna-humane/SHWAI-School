...
function NoticesPage() {
  ...
  const deleteMut = useMutation({
    mutationFn: (id: string) => deleteNotice({ data: { id } }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["notices"] });
  ...
  function openNotice(n: NoticeWithRead) {
    setSelected(n);
    if (!n.is_read) {
      markNoticeRead({ data: { noticeId: n.id } }).then(() =>
        qc.invalidateQueries({ queryKey: ["notices"] }),
      );
    }
  }
        ...
        <div className="space-y-1.5">
          ...
          <div className="flex items-center gap-2">
            ...
            {form.attachmentName && (
              <span className="flex items-center gap-1 text-xs text-muted-foreground">
                ...
                <button
                  type="button"
                  onClick={() => setForm((f) => ({ ...f, attachmentName: "", attachmentData: "" }))}
          ...
          <input
            ...
            onChange={(e) => {
              ...
              if (f)
                pickFile(f, (name, data) =>
                  setForm((frm) => ({ ...frm, attachmentName: name, attachmentData: data })),
                );
            }}
          />
        </div>
              ...
              >
                <div className="flex items-start gap-2">
                  ...
                  <div className={`min-w-0 flex-1 ${n.is_read ? "pl-3.5" : ""}`}>
                    <p
                      className={`truncate text-sm ${!n.is_read ? "font-semibold" : "font-medium"}`}
                    >
                      {n.title}
                    </p>
      ...
      >
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="space-y-1.5">
              <Label htmlFor="notice-desc">
                Notice Description <span className="text-destructive">*</span>
              </Label>
              <Textarea
                id="notice-desc"
                rows={4}
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={
                !form.title || !form.content || form.audience.length === 0 || createMut.isPending
              }
            >
        ...
        <DialogContent className="sm:max-w-lg">
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={
                !form.title || !form.content || form.audience.length === 0 || editMut.isPending
              }
            >
