...
The repository now includes a real production-code foundation for school onboarding at `/app/onboarding`, authorized multi-school membership switching, one-time email verification with rate-limited resend, password recovery with session revocation, TOTP MFA with encrypted secrets and one-time recovery codes, password rotation, idle session expiry, per-session revocation, controlled invitations at `/accept-invitation`, server-enforced permissions, server-derived plan/status entitlements, staged student CSV/JSON import at `/app/data-import`, bounded audited CSV/JSON export at `/app/data-export`, privacy request review at `/app/privacy`, owner system health and incident controls at `/app/system-health`, `/health` and `/readiness` probes, a shared `/ready`/`/readiness` handler, persistent idempotent job records, a secure job-runner boundary, persisted notification shell integration, a signed billing webhook boundary at `/api/billing/webhook`, names-only `.env.example`, a machine-readable `npm run readiness:check` deployment gate, and a GitHub Actions verification workflow.

These workflows are not claimed to be fully production-ready without deployment evidence. Email, private object storage, XLSX parsing, large background exports, durable workers, payment checkout/customer creation/reconciliation, SSO, Google/Microsoft education connectors, monitoring delivery, backups/PITR, restore testing, RLS review, WAF/DDoS controls, and production browser verification remain `CONFIGURATION REQUIRED`, `DEPLOYMENT REQUIRED`, `BLOCKED`, or `NOT VERIFIED`. TOTP and signed billing webhook code exist, but each still requires migrated PostgreSQL, production secret management, provider sandbox evidence, and authenticated verification. Read [`docs/PRODUCTION_READINESS_REPORT.md`](docs/PRODUCTION_READINESS_REPORT.md), [`docs/DEPLOYMENT.md`](docs/DEPLOYMENT.md), [`docs/INTEGRATIONS.md`](docs/INTEGRATIONS.md), and [`docs/BACKUP_AND_RECOVERY.md`](docs/BACKUP_AND_RECOVERY.md) before enabling a real school.
