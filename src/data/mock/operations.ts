...
export const FEE_RECORDS: FeeRecord[] = STUDENTS.slice(0, 60).map((s, i) => {
  ...
  const paid =
    status === "paid"
      ? total - concession
      : status === "partial"
        ? Math.round((total - concession) * 0.55)
        : 0;
  return {
    id: `fee-${n}`,
    ...
    status,
    method:
      paid > 0 ? (["UPI", "Net Banking", "Cash", "Cheque", "Card"] as const)[n % 5] : undefined,
    concession,
    scholarship: rnd(n * 4.1) > 0.9 ? "Merit Scholarship (State)" : undefined,
  };
});

export const FEE_COLLECTION_TREND = ["Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov"].map(
  (m, i) => ({
    month: m,
...
export const ADMISSIONS: AdmissionEnquiry[] = APPLICANT_NAMES.map((applicantName, i) => {
  ...
  return {
    ...
    documentsTotal: 6,
    entranceScore: ["entrance-test", "interview", "offer", "enrolled"].includes(stage)
      ? num(n * 7.4, 42, 96)
      : undefined,
...
export const LIBRARY_ITEMS: LibraryItem[] = [
  ...
  },
  {
    ...
    isbn: "978-81-7016-045-3",
    copies: 10,
    available: 4,
  },
  {
    id: "lib-6",
    title: "Physics for Class 11 (Vol. 1)",
    author: "H. C. Verma",
    category: "Reference",
    isbn: "978-81-7709-187-1",
    copies: 24,
    available: 1,
    issuedTo: "Ishita Menon",
    dueDate: "24/11/2025",
  },
  {
    id: "lib-7",
    title: "Atlas of India",
    author: "Orient BlackSwan",
    category: "Reference",
    isbn: "978-81-2504-560-4",
    copies: 30,
    available: 22,
  },
  {
...
export const INVENTORY_ITEMS: InventoryItem[] = [
  {
    id: "inv-1",
    name: "Chemistry lab beakers (250 ml)",
    category: "Lab",
    quantity: 84,
    reorderLevel: 40,
    unitCost: 120,
    location: "Science Block — Store 1",
  },
  {
    id: "inv-2",
    name: "Whiteboard markers (box of 10)",
    category: "Stationery",
    quantity: 22,
    reorderLevel: 30,
    unitCost: 340,
...
export const STAFF_LEAVE_REQUESTS = [
  ...
  },
  {
    ...
    to: "05/12/2025",
    days: 5,
    reason: "Village visit",
    status: "pending",
    substitute: "Sopan Tambe",
  },
  {
    id: "lv-4",
    name: "Ritu Agarwal",
