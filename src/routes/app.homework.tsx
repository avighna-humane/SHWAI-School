...
import { ParentHomeworkView } from "@/components/v2/parent-homework-view";
    ...
    mutationFn: (d: typeof form) =>
      createHomework({
        data: {
          ...
          referenceMaterial: d.referenceMaterial,
        },
      }),
    onSuccess: () => {
    ...
    <div className="space-y-6">
            ...
            <tbody>
              {homework.map((hw) => (
                ...
                >
                  ...
                  <td
                    className={`px-4 py-3 ${isOverdue(hw.due_date) ? "text-orange-600" : "text-muted-foreground"}`}
                  >
                    {fmtDate(hw.due_date)}
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <Button
                      ...
                      onClick={() => deleteMut.mutate(hw.id)}
                    >
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                onChange={(e) => setForm((f) => ({ ...f, referenceMaterial: e.target.value }))}
              />
            </div>
          </div>
          <DialogFooter>
            ...
            <Button
              disabled={!form.title || !form.classId || !form.dueDate || createMut.isPending}
              onClick={() => createMut.mutate(form)}
            >
              {createMut.isPending ? <Icons.Loader2 className="mr-2 size-4 animate-spin" /> : null}
              Create
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={gradeOpen} onOpenChange={setGradeOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Grade Submission — {selectedSub?.student_name}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-2">
            ...
            <div className="space-y-1.5">
              <Label>
                Grade (out of{" "}
                {homework.find((h) => h.id === selectedSub?.homework_id)?.total_marks ?? "?"})
              </Label>
              <Input
                type="number"
                min="0"
          ...
          {homework.map((hw) => {
            ...
            return (
              ...
              >
                ...
                <div
                  className={`mt-1.5 flex items-center gap-1.5 text-xs ${overdue ? "text-orange-500" : "text-muted-foreground"}`}
                >
                  <Icons.Calendar className="size-3" />
                  <span>Due {fmtDate(hw.due_date)}</span>
                </div>
                {hw.total_marks > 0 && (
                  <div className="mt-1.5 flex items-center gap-1.5 text-xs text-muted-foreground">
                    ...
                    <span>
                      {sub?.grade != null
                        ? `${sub.grade}/${hw.total_marks}`
                        : `${hw.total_marks} marks`}
                    </span>
                  </div>
                )}
                {sub?.feedback && (
                  <p className="mt-2 rounded-md bg-green-50 p-2 text-xs text-green-700 dark:bg-green-900/20 dark:text-green-400">
                    💬 {sub.feedback}
                  </p>
                )}
              </div>
      ...
      <Dialog open={submitOpen} onOpenChange={setSubmitOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-1 py-1">
            <p className="text-sm text-muted-foreground">
              {selectedHw?.subject} · {selectedHw?.class_label}
            </p>
            {selectedHw && (
              <p
                className={`text-xs ${isOverdue(selectedHw.due_date) ? "text-orange-500" : "text-muted-foreground"}`}
              >
                Due {fmtDate(selectedHw.due_date)}
              </p>
            )}
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex items-center gap-2">
                <Button
                  ...
                  type="button"
                >
                  <Icons.Paperclip className="mr-2 size-4" />
          ...
          <DialogFooter>
            ...
            <Button onClick={() => submitMut.mutate()} disabled={submitMut.isPending}>
              {submitMut.isPending ? <Icons.Loader2 className="mr-2 size-4 animate-spin" /> : null}
              Submit
            </Button>
          </DialogFooter>
        </DialogContent>
...
function HomeworkPage() {
  const { role, schoolId, userId, user } = useAppState();
  const props = { schoolId, role, userId, userName: user.name };
  if (role === "student") return <StudentView {...props} />;
  if (role === "parent") return <ParentHomeworkView />;
