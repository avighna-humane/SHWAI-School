...
export type AIErrorType =
  "auth" | "rate_limit" | "server" | "network" | "invalid_request" | "unknown";
...
export function classifyError(error: unknown): ClassifiedError {
  if (APICallError.isInstance(error)) {
    ...
    if (status === 401 || status === 403) {
      return {
        ...
        message: "Authentication failed (invalid, revoked, or unauthorized API key).",
      };
    }
    if (status === 429) {
      return {
  ...
  const err = error as
    { status?: number; statusCode?: number; code?: string; message?: string } | null | undefined;
  ...
  if (status !== undefined && status >= 500) {
    return {
      ...
      retryNextKey: true,
      cooldownMs: 5_000,
      message: err?.message ?? "Provider is temporarily unavailable.",
    };
  }
  if (status !== undefined && status >= 400) {
    return {
