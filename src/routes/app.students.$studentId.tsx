  ...
  if (!student) {
    return (
      <div className="flex flex-col items-center justify-center gap-4 py-20 text-center">
        ...
        <Button variant="outline" onClick={() => navigate({ to: '/app/students' })}>Back to Students</Button>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <button onClick={() => navigate({ to: '/app/students' })}
        ...
        <Icons.ChevronLeft className="size-4" />Back to Students
      </button>

      {/* Profile header */}
      <div className="flex items-start gap-4 rounded-xl border p-5">
        <Avatar className="size-16">
          <AvatarFallback style={{ backgroundColor: hsl(student.name), color: '#fff' }} className="text-xl font-bold">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="mt-1 flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-muted-foreground">
            <span className="flex items-center gap-1"><Icons.GraduationCap className="size-3.5" />{cls?.label ?? student.classId}</span>
            <span className="flex items-center gap-1"><Icons.Hash className="size-3.5" />{student.id}</span>
            <span className="flex items-center gap-1"><Icons.Users className="size-3.5" />{student.guardianName}</span>
          </div>
          <div className="mt-2 flex flex-wrap gap-2">
            <Badge variant="outline" className={`text-xs ${(student.attendancePct ?? 0) < 75 ? 'border-orange-500 text-orange-500' : 'border-green-600 text-green-600'}`}>
              <Icons.UserCheck className="mr-1 size-3" />{student.attendancePct ?? '—'}% attendance
            </Badge>
          </div>
        </div>
      </div>
      ...
      <Tabs defaultValue="submissions">
        ...
        <TabsContent value="chat" className="mt-4">
          <ChatTab
            ...
            myId={userId} myName={userName}
            partnerId={studentId} partnerName={student.name}
          />
        </TabsContent>
      </Tabs>
    </div>
...
function OverviewTab({ student, cls }: { student: any; cls: any }) {
  const rows = [
...
function SubmissionsTab({ studentId, schoolId }: { studentId: string; schoolId: string }) {
  const { data: submissions = [], isLoading } = useQuery({
    ...
    queryFn: () => listStudentSubmissions({ data: { studentId, schoolId } }),
  });

  if (isLoading) return <div className="flex h-32 items-center justify-center text-muted-foreground"><Icons.Loader2 className="mr-2 size-4 animate-spin" />Loading…</div>;
    ...
    <div className="overflow-x-auto rounded-xl border">
      <table className="w-full text-sm">
        <thead className="border-b bg-muted/50">
          <tr>{['Homework', 'Subject', 'Submitted', 'Status', 'Grade', 'Feedback', ''].map(h => (
        ...
        <tbody>
          {submissions.map(s => (
            <tr key={s.id} className="border-b transition-colors last:border-0 hover:bg-muted/30">
              ...
              <td className="px-4 py-3 font-medium">{s.grade != null ? s.grade : <span className="text-muted-foreground">—</span>}</td>
              <td className="px-4 py-3 max-w-[160px]">
                {s.feedback ? <span className="truncate text-xs text-muted-foreground">{s.feedback}</span> : <span className="text-muted-foreground">—</span>}
              </td>
              <td className="px-4 py-3">
                {s.file_name && (
                  <Button variant="ghost" size="icon" className="size-8" title={s.file_name}
                    onClick={() => { const a = document.createElement('a'); a.href = `data:${s.file_type};base64,${s.file_data}`; a.download = s.file_name; a.click(); }}>
...
function ChatTab({ schoolId, role, myId, myName, partnerId, partnerName }: {
  schoolId: string; role: string; myId: string; myName: string; partnerId: string; partnerName: string;
}) {
  ...
  const { data: messages = [] } = useQuery({
    ...
    queryFn: () => getMessages({ data: { schoolId, userId: myId, partnerId } }),
    refetchInterval: 5000,
  });

  const sendMut = useMutation({
    mutationFn: () => sendMessage({ data: { schoolId, senderId: myId, senderName: myName, senderRole: role, receiverId: partnerId, receiverName: partnerName, body } }),
    onSuccess: () => { setBody(''); qc.invalidateQueries({ queryKey: ['messages'] }); setTimeout(() => endRef.current?.scrollIntoView({ behavior: 'smooth' }), 100); },
  ...
  return (
    <div className="flex h-96 flex-col rounded-xl border overflow-hidden">
        ...
        {messages.length === 0 ? (
          ...
          return (
            <div key={m.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[75%] rounded-2xl px-4 py-2.5 text-sm ${isMe ? 'rounded-tr-sm bg-primary text-primary-foreground' : 'rounded-tl-sm bg-muted'}`}>
                <p>{m.body}</p>
                <p className={`mt-1 text-[10px] ${isMe ? 'text-primary-foreground/70' : 'text-muted-foreground'}`}>{fmtDateTime(m.created_at)}</p>
              </div>
            </div>
          );
        })}
      ...
      <div className="border-t p-3 flex items-center gap-2">
        <Input className="flex-1" placeholder="Type a message…" value={body}
          onChange={e => setBody(e.target.value)}
          onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey && body.trim()) { e.preventDefault(); sendMut.mutate(); }}}
        />
        <Button size="icon" disabled={!body.trim() || sendMut.isPending} onClick={() => sendMut.mutate()}>
          {sendMut.isPending ? <Icons.Loader2 className="size-4 animate-spin" /> : <Icons.Send className="size-4" />}
        </Button>
      </div>
    </div>
  );
