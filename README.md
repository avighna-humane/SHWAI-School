...
The repository now includes a real production-code foundation for school onboarding at `/app/onboarding`, authorized multi-school membership switching, one-time email verification with rate-limited resend, password recovery with session revocation, TOTP MFA with encrypted secrets and one-time recovery codes, password rotation, idle session expiry, per-session revocation, controlled invitations at `/accept-invitation`, server-enforced permissions, server-derived plan/status entitlements, staged student CSV/JSON import at `/app/data-import`, queued audited CSV/JSON export at `/app/data-export` with private signed artifacts, tenant-safe document upload/download/delete actions, privacy request review at `/app/privacy`, owner system health and incident controls at `/app/system-health`, `/health` and `/readiness` probes, a shared `/ready`/`/readiness` handler, persistent idempotent job records with leases/retry/dead-letter state, a deployable `npm run worker` process, signed billing webhook processing at `/api/billing/webhook`, names-only `.env.example`, machine-readable `npm run readiness:check` and `npm run db:verify` deployment gates, `npm run db:backup` and isolated `npm run db:restore:test` operations, and a GitHub Actions verification workflow.

These workflows are not claimed to be fully production-ready without deployment evidence. Email, private object storage/scanning, XLSX parsing, teacher/parent/staff import adapters, durable worker hosting, payment checkout/customer creation/reconciliation, SSO, Google/Microsoft education connectors, monitoring delivery, backups/PITR, restore testing, RLS review, WAF/DDoS controls, and production browser verification remain `CONFIGURATION REQUIRED`, `DEPLOYMENT REQUIRED`, `BLOCKED`, or `NOT VERIFIED`. TOTP and signed billing webhook code exist, but each still requires migrated PostgreSQL, production secret management, provider sandbox evidence, and authenticated verification. Read [`docs/PRODUCTION_READINESS_REPORT.md`](docs/PRODUCTION_READINESS_REPORT.md), [`docs/PRODUCTION_SETUP.md`](docs/PRODUCTION_SETUP.md), [`docs/DEPLOYMENT.md`](docs/DEPLOYMENT.md), [`docs/INTEGRATIONS.md`](docs/INTEGRATIONS.md), and [`docs/BACKUP_AND_RECOVERY.md`](docs/BACKUP_AND_RECOVERY.md) before enabling a real school.
...
All V3 server functions derive identity, role, and school from the authenticated HTTP-only session. Server-side policy enforces student-versus-teacher feature permissions, tenant scoping, prompt safety checks, a 12,000-character input limit, an eight-request-per-minute per-user limit, a 2,000-request-per-school daily limit, a school-owned daily token budget, bounded output tokens, usage records, audit records, and schema validation. The tutor does not expose unrelated personal data to the provider, and stored source material is treated as data rather than executable instructions.
...
`src/config/navigation.ts` declares every module with `roles` and an optional `plan`; the sidebar, command palette, and mobile navigation derive from it. The server derives `user → school → membership → role → plan → subscription status` from the HTTP-only session cookie and PostgreSQL. The browser cannot invent a role or school; it may switch only among active server-authorized memberships through session rotation. Passwords use PBKDF2-SHA-256, sessions expire after eight hours of absolute time or two hours idle, and enabled accounts can require TOTP at login. Production deployment still requires PostgreSQL, TLS, secret rotation, monitoring, and an operational identity lifecycle.

## Subscription feature gating

`src/config/plans.ts` defines Starter, Professional, and Enterprise feature gates. `planAllows(current, required)` drives locked-feature states and upgrade prompts, while `requireFeatureEntitlement` enforces plan plus reconciled subscription status on the server for AI generation and data portability. V3 AI routes are marked Professional in navigation, while database and provider availability remain infrastructure requirements rather than simulated plan entitlements.
