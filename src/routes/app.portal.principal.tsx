      ...
      <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as any)} className="space-y-4">
        <TabsList className="flex flex-wrap rounded-xl bg-card p-1 shadow-sm border border-border/50">
          <TabsTrigger value="overview" className="text-xs gap-1.5">
            <Sparkles className="size-3.5" /> School Overview
          </TabsTrigger>
          <TabsTrigger value="notices" className="text-xs gap-1.5">
            <Megaphone className="size-3.5" /> General Notices ({schoolNotices.length})
          </TabsTrigger>
          <TabsTrigger value="teacher-posts" className="text-xs gap-1.5">
            <Presentation className="size-3.5" /> Teacher-Only Posts ({teacherPosts.length})
          </TabsTrigger>
        </TabsList>
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  Average school-wide attendance this term.
                </p>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  All grade curriculums are running fully on track.
                </p>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  Average classroom homework completion rate.
                </p>
              </CardContent>
            </Card>
          </div>
          ...
          <Card>
            <CardHeader className="flex flex-row justify-between items-center pb-3">
              <div>
                ...
                <CardDescription>
                  Notices and circulars published school-wide for parents and students.
                </CardDescription>
              </div>
            ...
            <CardContent>
              {noticesQuery.isLoading ? (
                ...
                <EmptyState
                  ...
                  description="You have not published any general school notices."
                ...
                <div className="space-y-4">
                  {schoolNotices.map((n: any) => (
                    <div
                      key={n.id}
                      className="p-4 border rounded-xl flex justify-between items-start gap-4 bg-card hover:bg-muted/10 transition-colors"
                    >
                      <div className="space-y-1.5">
                        <h3 className="font-bold text-base text-primary">{n.title}</h3>
                        <p className="text-sm text-muted-foreground">{n.body}</p>
                        <p className="text-[11px] text-muted-foreground mt-1 flex items-center gap-1">
                          <CalendarClock className="size-3.5" /> Published on{" "}
                      ...
                      <div className="flex items-center gap-3 shrink-0">
                        <div className="text-right text-xs text-muted-foreground mr-1">
                          <p className="font-bold text-foreground">
                            Opened {n.viewedCount ?? 0} / {n.recipientCount ?? 0}
                          </p>
                          <button
                            onClick={() => setActivityNoticeId(n.id)}
                            className="text-primary hover:underline font-semibold text-[10px] mt-1 block"
                          >
                            Track Read
                          </button>
                        </div>
          ...
          <Card>
            ...
            <CardContent>
              {teacherPostsQuery.isLoading ? (
                ...
                <EmptyState
                  title="No staff posts published"
                  description="You have not published any posts specifically for teachers."
                  icon={<Megaphone className="size-6" />}
                />
              ) : (
                <div className="space-y-4">
                  {teacherPosts.map((n: any) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3 shrink-0 ml-4">
                        <div className="text-right text-xs text-muted-foreground mr-1">
                          <p className="font-bold text-foreground">
                            Read: {n.viewedCount ?? 0} / {n.recipientCount ?? 0}
                          </p>
                          <button
                            onClick={() => setActivityNoticeId(n.id)}
                            className="text-primary hover:underline font-semibold text-[10px] mt-1 block"
        ...
        <DialogHeader className="flex flex-row items-center justify-between border-b pb-3">
          <div>
            ...
            <DialogDescription className="text-xs">
              Detailed audit register of opened notices.
            </DialogDescription>
          </div>
