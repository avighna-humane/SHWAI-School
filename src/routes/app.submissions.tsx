...
import { listAllSubmissions, gradeSubmission, type SubmissionWithHomework } from '@/actions/homework';
...
function SubmissionsPage() {
  ...
  const gradeMut = useMutation({
    mutationFn: () => gradeSubmission({ data: { submissionId: selected!.id, grade: gradeVal ? Number(gradeVal) : null, feedback: feedbackVal, role } }),
  ...
  return (
    <div className="space-y-6">
      <header>
        ...
        <p className="mt-1.5 max-w-2xl text-sm leading-6 text-muted-foreground">Review and grade student homework submissions.</p>
      </header>

      <div className="flex items-center gap-3">
        <div className="relative flex-1 max-w-sm">
          <Icons.Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" />
          <Input className="pl-9" placeholder="Search student or homework…" value={search} onChange={e => setSearch(e.target.value)} />
        </div>
        <Badge variant="outline" className="rounded-full">{filtered.length} submissions</Badge>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center gap-3 rounded-xl border border-dashed py-20 text-center">
          ...
          <p className="text-sm text-muted-foreground">Submissions will appear here once students submit their homework.</p>
        </div>
      ) : (
        <div className="overflow-x-auto rounded-xl border">
          <table className="w-full text-sm">
            <thead className="border-b bg-muted/50">
              <tr>{['Student', 'Homework', 'Subject', 'Class', 'Submitted', 'Status', 'Grade', ''].map(h => (
                ...
                <tr key={s.id} className="border-b transition-colors last:border-0 hover:bg-muted/30">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1">
                      {s.file_name && (
                        <Button variant="ghost" size="icon" className="size-8" title="Download attachment"
                      ...
                      <Button variant="ghost" size="sm" onClick={() => openGrade(s)}>
                        <Icons.PenLine className="mr-1.5 size-3.5" />Grade
                      </Button>
                    </div>
                  </td>
                </tr>
      ...
      <Dialog open={gradeOpen} onOpenChange={setGradeOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader><DialogTitle>Grade — {selected?.student_name}</DialogTitle></DialogHeader>
          <div className="space-y-4 py-2">
            <div className="rounded-lg bg-muted/50 p-3">
              <p className="text-sm font-medium">{selected?.homework_title}</p>
              <p className="text-xs text-muted-foreground">{selected?.homework_subject} · {selected?.class_label}</p>
            </div>
            {selected?.comment && (
              <div className="rounded-lg bg-muted p-3 text-sm"><span className="font-medium">Student note: </span>{selected.comment}</div>
            )}
            {selected?.file_name && (
              <div className="flex items-center gap-2 rounded-lg border p-3">
                ...
                <Button size="sm" variant="outline" onClick={() => {
                  ...
                  a.download = selected.file_name; a.click();
                }}>Download</Button>
              </div>
            )}
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <Label>Grade</Label>
                <Input type="number" min="0" value={gradeVal} onChange={e => setGradeVal(e.target.value)} placeholder="Marks" />
              </div>
            </div>
            <div className="space-y-1.5">
          ...
          <DialogFooter>
            ...
            <Button onClick={() => gradeMut.mutate()} disabled={gradeMut.isPending}>
              {gradeMut.isPending ? <Icons.Loader2 className="mr-2 size-4 animate-spin" /> : null}Save
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
